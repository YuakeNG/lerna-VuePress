 ...
 * @param cwd
 * @param pkg
 * @returns yuake-fe-eslint-config/index
 * @returns yuake-fe-eslint-config/react
 * @returns yuake-fe-eslint-config/typescript/index
 * @returns yuake-fe-eslint-config/typescript/react
 */
export function getESLintConfigType(cwd: string, pkg: PKG): string {
  ...
  return (
    'yuake-fe-eslint-config/' + `${language}/${dsl}`.replace(/\/$/, '/index').replace(/^\//, '')
  );
}
